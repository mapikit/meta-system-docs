...
- Currently Meta-System is at version 0.4, which means the API might receive breaking changes faster than you expect. The team will make conscious effort, though, to only make such changes if there is tangible benefits in doing so.
